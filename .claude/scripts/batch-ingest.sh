...
if [[ -z "${VAULT_NAME:-}" ]]; then
    source "${REPO_ROOT}/.claude/scripts/config.sh" 2>/dev/null || true
fi

# Per-vault state dir if VAULT_NAME resolved; fall back to repo-level log.
if [[ -n "${VAULT_NAME:-}" ]]; then
    LOG_DIR="${REPO_ROOT}/vaults/${VAULT_NAME}/state/logs"
    mkdir -p "$LOG_DIR"
    LOG="${LOG_DIR}/batch-ingest.log"
else
    LOG="${REPO_ROOT}/.claude/scripts/batch-ingest.log"
fi
...
while IFS= read -r VID; do
    ...
    OUT=$("${REPO_ROOT}/.claude/scripts/ingest.sh" "$VID" 2>&1)
    RC=$?
    case $RC in
        0)
            if [[ "$OUT" == SKIP* ]]; then
