...
1. SOLO puedes LEER de:
   - {REPO}/tests/raw-responses/{LABEL}/run-{N}/Q*.json (las 15 respuestas a evaluar)
   - {REPO}/tests/prompts/O4v2/questions.yaml (battery con expected_atoms / expected_resolution / expected_temporal_narrative por pregunta)
   - {VAULT}/meta/contradictions.md (referencia de conflictos documentados)
   - {VAULT}/notes/*.md (verificación de atoms citados)
   - {VAULT}/MOC/*.md (verificación de topics)
...
4. Escribes UN único archivo JSON en EXACTAMENTE esta ruta:
   {REPO}/tests/raw-responses/{LABEL}/run-{N}/evaluation.json
...
Guarda el JSON en: {REPO}/tests/raw-responses/{LABEL}/run-{N}/evaluation.json
